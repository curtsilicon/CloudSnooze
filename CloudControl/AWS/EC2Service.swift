...
final class EC2Service {
    ...
    func describeInstances(credentials: AWSCredentials) async throws -> [EC2Instance] {
        ...
        let data = try await client.execute(req)
        return try parseDescribeInstances(data: data, region: credentials.region)
    }
        ...
        return await withTaskGroup(of: [EC2Instance].self) { group in
            for region in regions {
                group.addTask {
                    ...
                    let query = "Action=DescribeInstances&Version=2016-11-15"
                    let req   = self.client.signedRequest(
                        method:      "GET",
                        service:     "ec2",
                        region:      region,
                        host:        host,
                    ...
                    do {
                        let data      = try await self.client.execute(req)
                        let instances = try self.parseDescribeInstances(data: data, region: region)
    ...
    private func parseDescribeInstances(data: Data, region: String) throws -> [EC2Instance] {
...
private final class EC2InstancesParser: NSObject, XMLParserDelegate {
    ...
    private func flushInstance() {
        guard let instanceId = current["instanceId"], !instanceId.isEmpty else {
            print("[EC2Parser] skipping item — no instanceId. keys: \(current.keys.sorted())")
            return
        }
        ...
        print("[EC2Parser] parsed: \(instanceId) name=\(nameTag) state=\(state.rawValue) region=\(region)")
